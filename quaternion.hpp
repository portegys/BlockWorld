...
class cQuaternion
{
 ...
 public:
  ...
  // Build a rotation matrix, given a quaternion rotation.
  void build_quat(GLfloat q[4], GLfloat m[4][4]);
...
void cQuaternion::axis_to_quat(GLfloat a[3], GLfloat phi, GLfloat q[4])
{
    vnormal(a);
    vcopy(a,q);
    vscale(q,(GLfloat)sin(phi/2.0f));
    q[3] = (GLfloat)cos(phi/2.0f);
}
...
/*
 * Build a quaternion, given a rotation matrix.
 */

void cQuaternion::build_quat(GLfloat q[4], GLfloat m[4][4])
{
	GLfloat *mr = &m[0][0];
	GLfloat t,s,s0,s1,s2;

	if ((t = (mr[0] + mr[5] + mr[10] + 1.0)) > 0.0)
	{
		s = 0.5 / sqrt(t);
		q[3] = 0.25 / s;
		q[0] = ( mr[9] - mr[6] ) * s;
        ...
        s0  = sqrt( 1.0 + mr[0] - mr[5] - mr[10] ) * 2.0;
        s1  = sqrt( 1.0 + mr[5] - mr[0] - mr[10] ) * 2.0;
        s2  = sqrt( 1.0 + mr[10] - mr[0] - mr[5] ) * 2.0;
...
{
	quat[0] = axis[0] * (GLfloat)sin(angle / 2.0f);
	quat[1] = axis[1] * (GLfloat)sin(angle / 2.0f);
	quat[2] = axis[2] * (GLfloat)sin(angle / 2.0f);
	quat[3] = (GLfloat)cos(angle / 2.0f);
	normalize_quat(quat);
}
...
void cQuaternion::mergeRotation(GLfloat angle, GLfloat *axis)
{
	...
	q2[0] = axis[0] * (GLfloat)sin(angle / 2.0f);
	q2[1] = axis[1] * (GLfloat)sin(angle / 2.0f);
	q2[2] = axis[2] * (GLfloat)sin(angle / 2.0f);
	q2[3] = (GLfloat)cos(angle / 2.0f);
	normalize_quat(q2);
	q1[0] = quat[0];
	q1[1] = quat[1];
	q1[2] = quat[2];
